...
var juego = {
    ...
    get_preguntas: function( id_nivel, splash, destacado ){
    	...
    	var action_ajax = 'get_preguntas_chapiniveles';
    	if( destacado ){
            ...
            success: function(a){
                ...
                $('.nivelBtnDestacado').click(function(){
                    ...
                    if(bloqueado!=true){
                        ...
                        splash.show();
                        juego.get_preguntas(id_nivel, splash, true);
...
var niveles={
    ...
    getNiveles: function(){
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: {
                action: 'get_chapiniveles_con_splash',//'get_niveles',
                user_email: user.correo,
                user_pass: user.password,
            ...
            success: function(a){
                ...
                niveles.splashes = $(".splashChapinivel");
                $('.nivelBtn').click(function(){
                    var bloqueado = $(this).hasClass('nivel_bloqueado');
                    if(bloqueado!=true){
                        var id_nivel = $(this).data('nivelid');
                        var splash = $(niveles.splashes[niveles.contador_splashes]);
                        splash.show();
                        juego.get_preguntas(id_nivel, splash, false);
                        niveles.contador_splashes ++;
                        console.log("contador splahes: "+niveles.contador_splashes);
                        console.log("cant splahes: "+niveles.splashes.length);
                        if( niveles.contador_splashes >= niveles.splashes.length ){
                            console.log('entro contador splahses = 0');
                            niveles.contador_splashes = 0;
                        }
                    }
                });
...
var fb = {
	...
	login: function(){
		...
		facebookConnectPlugin.getLoginStatus(function(response) {
		  navigator.notification.alert("exito "+JSON.stringify(response));
		  ...
		  if (response.status === 'connected') {
			  ...
			  facebookConnectPlugin.login(["public_profile","email"],
			     ...
			     myModal.open('Error','Ha ocurrido un error al conectarse a Facebook. Por favor intenta de nuevo');
				 console.log(""+JSON.stringify(error));
				 navigator.notification.alert(JSON.stringify("error"+error));
			 });
	},
	getInfo: function(){
